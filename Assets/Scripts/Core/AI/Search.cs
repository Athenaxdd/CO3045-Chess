	...
	public class Search
	{
		...
		public void StartSearch()
		{
			...
			if (settings.useIterativeDeepening)
			{
				...
				Debug.Log("Iterative deepening to depth " + targetDepth + " with endless search mode: " + settings.endlessSearchMode);
		...
		int SearchMoves(int depth, int plyFromRoot, int alpha, int beta)
		{
			...
			if (plyFromRoot > 0)
			{
				...
				if (alpha >= beta)
				{                                           //The function returns alpha since it means the search has been cut off
					return alpha;
				}
			}
